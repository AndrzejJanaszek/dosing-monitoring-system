...
Po co?
Program da możliwośc diagnozy wagi oraz obeiktu poprzed dostęp do zapisywanych danych.
...
Dzięki zapisowi zdarzeń bedzie można potencjalnie wykryć lub wykryć wcześniej wadę wagi, czy fizyczną usterkę obiektu poprzez porównanie danych zapisanych przez program do wartości, które powinny być w teorii np.:
- powinni przywieźć 40 ton cementu a system pokazuje że zasypane zostało 35 ton. (być może waga nie pokazuje prawidłowo, być może ktoś próbował oszukać betoniarnię na zakup cementu)
- średnio na gruszkę powinniśmy sypać 5 ton cementu, zrobiliśmy w dzień 10 gruszek i okazuje się że z wyliczeń dozowaliśmy nie 50 a 60 ton (być może problem z wagą a być może problem z mechanizmem dozowania - np. zepsuta śróba zsypu)
...
np. Zamiast zorientować się że mechasnizm dozowania psuje się, w momencie kiedy zbiornik jest pusty, lub jest w nim np. 20t zamiast 40t co oznaczałoby że problem występuje już dłuższy czas, instniałaby możliwość zaobserwowania tego problemu wcześniej.


---

Opis obiektu:
`N` opomiarowanych silosów. 
Na każdy zbiornik (może się zdarzyć, że jeden silos ma w sobie dwa zbiorniki) przypada wskazanie z miernika, przesyłane po RS232 oraz dwa sygnały cyfrowe mające wartość 1 lub 0 w zależności od tego czy zdarzenie ma miejsce czy nie.
...
Komputer docelowy odpowiedzialny jest za odczyt danych z mierników (po RS232, zastosowanie przejściówki RS232-USB) oraz arduino. Komputer docelowy przetwarza te dane, zapisuje, gromadzi i umożliwia ich analizę.
